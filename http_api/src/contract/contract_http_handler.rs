...
pub struct ContractHttpHandler {
    store_client_manager: Arc<Mutex<dyn StoreClientManager>>,
    sandbox_client_request_manager: Arc<dyn RequestManager>,
}

impl ContractHttpHandler {
    pub fn new(store_client_manager: &Arc<Mutex<dyn StoreClientManager>>, sandbox_client_request_manager: &Arc<dyn RequestManager>) -> Result<Arc<dyn HttpHandler>,Box<dyn std::error::Error>> {
        Ok(Arc::new(ContractHttpHandler{store_client_manager: store_client_manager.clone(), sandbox_client_request_manager: sandbox_client_request_manager.clone()}))
    }
    pub fn get_contract(&self,_req: &HttpApiRequest) -> Result<String,Box<dyn std::error::Error>> {
...
#[async_trait]
impl HttpHandler for ContractHttpHandler {
    async fn process(&self, _req: HttpApiRequest) -> Result<HttpApiResponse,Box<dyn std::error::Error>> {
        let contact = self.get_contract(&_req)?;
        let http_request = self.sandbox_client_request_manager.create_http_request()?;
        
        Ok(HttpApiResponse{
            body: String::from("bob"),
